...
async function loadMobilenet() {
  ...
  return tf.model({inputs: mobilenetModel.inputs, outputs: layer.output});
}
...
async function train() {
  ...
  model = tf.sequential({
    layers: [
      tf.layers.flatten({inputShape: [7, 7, 1024]}), // Updated input shape
  ...
  let loss = 0;
  await model.fit(dataset.xs, dataset.ys, { // Added await
    epochs: 10,
    callbacks: {
